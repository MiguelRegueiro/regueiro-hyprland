...
Item {
    ...
    function detectAuthenticationError(error) {
        ...
        return root.detectPasswordRequired(error)
            || lower.includes("invalid secrets")
    ...
    function connect(ssid, password, callback) {
        ...
        const existingProfile = root._pendingPassword.length > 0
            ? root.findSavedProfileName(target)
            : ""
    ...
    function _cleanupFailedProfile(result) {
        ...
        const profileName = savedProfile.length > 0
            ? savedProfile
            : (root._pendingPassword.length > 0 ? root._pendingSsid : "")
